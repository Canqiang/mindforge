...
本文档记录 MindForge 的产品定位、开放风险、范围、技术决策与路线图。每条具体决策见 [`adr/`](../adr)。AI / vibe coding 执行规则见 [`VIBE_CODING_RULES.zh-CN.md`](./VIBE_CODING_RULES.zh-CN.md)，根目录 [`AGENTS.md`](../AGENTS.md) 是给 agent 的最小执行契约。
...
Operation（insertNode / moveNode / editContent / setSelection / ...）
        │
        ▼
core.applyOp(doc, op)  ──►  doc'  （immutable，经 Immer）
        │
        ├──► 撤销栈
        │
        ▼
Zustand store 更新
        │
        ├──► layout/   重算位置（按 parentId + childIds memo）
        │
        ├──► render/   重渲染受影响节点（React 按 NodeId keyed）
        │
        └──► outline/  从 doc' 同步 Tiptap 状态（带 origin 标记防回环）
```

两个渲染目标（画布 + 大纲）共享一份事实来源（`doc`）；`Operation` 是唯一变更入口。这是「每加一个功能都不会让代码变意大利面」的核心不变量。
